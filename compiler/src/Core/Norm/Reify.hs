...
reifyNf :: Int -> Int -> V.Expr -> V.Type -> N.Nf
reifyNf tlen len t = \case
  V.TFunc _ a b -> N.ELam (reifyType' tlen a) $ reifyNf tlen (len + 1) (app a b t $ V.EVar len) b
  V.TForall {} -> undefined
  V.TVar {} -> undefined
  _ -> N.ENeu $ reifyExpr tlen len t

reifyExpr :: Int -> Int -> V.Expr -> N.Ne
reifyExpr tlen len =
  let re = reifyExpr tlen len
   ...
   in \case
        ...
        V.ELetPair q a t u -> N.ELetPair q (tre a) (re t) (frcPair u)
